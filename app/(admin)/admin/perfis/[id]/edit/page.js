...
import { Accordion, AccordionBody, AccordionHeader, AccordionItem , Row, Spinner,
...
export default function Edit() {
   ...
   function getProfile(id){
       ...
       .then(({ status, body}) => {
          ...
          switch(status){
              case 200:
               setOpen("first_open");
               setData({...data,permissions: body.permissions.map((p) => {return {id: p.id, name: p.name}})});
   ...
   function submit(formData){
      ...
      fetch(Constantes.urlBackAdmin + `profile/${params.id}`, {
            method: "PUT",
            headers: {
                "Module": "ADMINISTRATION",
                "Authorization": token2,
   ...
   useEffect(() => {
      console.log(data);
      if(data.first_open == undefined) toggle(open == "first_open" ? "administration" : open);
   },[data]);
       ...
       { loadingSubmit && <LoadingGif/>}
        ...
        <CardHeader className={styles.header} style={{justifyContent:"flex-start", alignItems: "center"}}> 
            <IoArrowBackCircleSharp style={{width:"45px",height:"70px",color:"#009E8B",cursor:"pointer"}}
                                    onClick={() => {router.back()}}/>
         <h1 className={styles.header_h1}>Edição de Perfil</h1>
        </CardHeader>
