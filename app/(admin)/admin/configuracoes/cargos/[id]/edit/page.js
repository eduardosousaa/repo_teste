...
import FormCargo from "../../../../../../../src/Components/Forms/FormCargo";
...
export default function Edit() {
   ...
   function submit(data){
      ...
      fetch(Constantes.urlBackAdmin + `position/${params.id}`, {
            method: "PUT",
            headers: {
      ...
      .then(({status, body}) => {
           ...
           switch(status){
               case 201:
                 ...
                 router.push("/admin/configuracoes/cargos");
               break;
               case 400:
                 setErrors(body);
                 showAlert("danger", "Preencha os dados obrigatórios!");
   ...
   { loading == true  ? <LoadingGif/> : <>
        ...
        <CardHeader className={styles.header} style={{justifyContent:"flex-start", alignItems: "center"}}>
           <IoArrowBackCircleSharp style={{width:"45px",height:"70px",color:"#009E8B",cursor:"pointer"}}
                                   onClick={() => {router.back()}}/>
           <h1 className={styles.header_h1}>Edição de Cargo</h1>
        </CardHeader>
        
        <CardBody style={{width:"90%"}}>
            <UncontrolledAccordion defaultOpen={["configuracao"]} stayOpen style={{flex:"1"}}>
               <AccordionItem>
                  <AccordionHeader /* targetId="configuracao" */><span className={styles.accordionTitle}>Dados do Cargo</span></AccordionHeader>
                  <AccordionBody accordionId="configuracao" style={{padding:"15px"}}>
                    <FormCargo data={data} submitData={submit} apiErrors={errors}/>
                  </AccordionBody>
               </AccordionItem>
