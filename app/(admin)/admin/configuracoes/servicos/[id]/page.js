   ...
   return (<>
       ...
       <CardHeader className={styles.header} style={{justifyContent:"flex-start", alignItems: "center"}}>
           <IoArrowBackCircleSharp style={{width:"45px",height:"70px",color:"#009E8B",cursor:"pointer"}}
                                   onClick={() => {router.back()}}/>
           <h1 className={styles.header_h1}>Dados de Categoria de Serviço</h1>
       </CardHeader>
       ...
       <CardBody style={{width:"90%"}}>
          <Row className="d-flex mt-3">
            ...
            <div style={{fontSize: "1.25rem",marginTop:"10px",marginBottom:"20px"}}>CatSers</div>
            { data.catSers.map((cat,index) => 
             <Card key={index} style={{ flex:"1", padding: "15px"}}>
                <Row className="d-flex mt-3">
                   <Col sm="12">    
                     <Label style={{height:"25px",fontSize:"16px",fontWeight:"bold"}}>Nome</Label><br/>
                     <Label style={{height:"25px",fontSize:"16px"}}>{cat.description}</Label> 
                   </Col>
                </Row>
               </Card>
            )}
