...
export default function Create() {
   ...
   const supplierOptions = (teste) => {
    ...
    const fuelSupplyOptions = (teste) => {
       ...
       query.supplierId = getValues("supplierId") || "";
                ...
                switch(status){
                    case 200:
                       for (const [key, value] of Object.entries(body)) {
                        ...
                        }else if(key == "fuelCost" || key == "priceLiter" || key == "invoiceValue"){
                           setValue(key, value ? FormatarReal(String(value.toFixed(2))) : "");
                        }else if(key == "quantityLiters"){
                           setValue(key,value ? String(value).replace(".",",") : "");
                        }else if(key == "invoiceId"){
                           setCreateInvoice(value == null);
                           setValue(key,value);
                        }else if(key == "vehicleMileage"){
                           setMileage(value);
                           setValue(key,value);
                        }else{
                           setValue(key,value);
                ...
                switch(status){
                    case 200:
                       for (const [key, value] of Object.entries(body)) {
                         ...
                         }else if(key == "supply" && value.length > 0){
                           setFuelSupplies(value.map((supply) => {
                              return { value:  supply.id,
                                       label: supply.plate +  " - " + FormatarData(supply.date,"dd/MM/yyyy") + " - R$" + FormatarReal(supply.fuelCost.toFixed(2)),
                                       vehicle: supply.plate,
                                       /* kilometer: supply.kilometer, */
                                       gasStation: supply.supplierName,
                                       fuelType: supply.fuelType,
                                       date: supply.date,
                                       valuePrice: supply.fuelCost,}
                           }));
                         }else if(key == "path" && value != null){ 
                           setValue(key,value);
                           setValue("invoiceDoc","unchanged");
                           setInvoiceDocBlob(Constantes.urlImages + value);
                         }else{
                           setValue(key,value);
                         } 
                       }
   ...
   function submit(data){
      ...
      if(type == "abastecimento") {
         ...
         if(data.invoiceDoc && data.invoiceDoc != "unchanged"){
            formData.append("invoiceDoc",data.invoiceDoc,"nota_fiscal.pdf"); 
         }

         url = `supply/invoice/${params.id}`;
      }
              ...
              <Row className="d-flex mt-3">
                ...
                <Col sm="4">
                   <AsyncSelectForm
                     ...
                     register={register}
                     required={false}
                     defaultValue={{value:getValues("invoiceId"),label:  "N°" + getValues("invoiceNumber") + " - R$" + getValues("invoiceValue")}}
                     onChange={(e) => {if(e){
                 ...
                 <Row className="d-flex mt-3">
                    <Col sm="6">
                       <AsyncSelectForm
                         ...
                         register={register}
                         required={true}
                         defaultValue={{value:getValues("vehicleId"),label:getValues("plate")}}
                         onChange={(e) => {setValue('vehicleId',e ? e.value : "");
                                           setValue('plate',e ? e.label : "");
                                           setMileage(e.mileage)}}
