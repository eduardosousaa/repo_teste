...
export default function SideBar({
  ...
  const renderMenuItem = (item: MenuItem) => {
    ...
    return (
      <div key={item.path}>
        <div
          className={`flex items-center justify-between py-3 px-4 cursor-pointer text-sm transition-all duration-200 mx-2 rounded-lg ${
            isActive
          ...
          {hasChildren && isOpen && (
            ...
            >
              {item.children!.map((sub, index) => (
                <div
                  key={`${sub.path}-${index}`}
                  className={`py-2 px-4 cursor-pointer text-sm rounded-lg mx-2 transition-all duration-200 ${
                    activePath === sub.path ||
                    activePath.startsWith(sub.path)
        ...
        {menuGroups.map((group) => {
          ...
          return (
            <div key={group.title} className="mb-6">
              <div
                className="flex items-center justify-between cursor-pointer bg-gray-600 text-white text-sm font-semibold py-3 px-4 mx-2 rounded-lg shadow-sm hover:bg-gray-700 transition-colors duration-200"
