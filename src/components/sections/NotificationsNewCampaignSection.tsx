...
interface NotificationsNewCampaignSectionProps {
  onDataChange: (regras: RegraData[]) => void
}

export default function NotificationsNewCampaignSection({ onDataChange }: NotificationsNewCampaignSectionProps) {
  ...
  // Busca as tabelas e colunas diretamente da API externa
  useEffect(() => {
    const fetchTabelasColunas = async () => {
      try {
        ...
        const response = await fetch('https://api-gedai.smartdatasolutions.com.br/notificacoes/campanha/listar-tabelas-colunas', {
          method: 'GET',
          headers: {
        ...
        console.log('Tabelas e colunas carregadas com sucesso')
      } catch (error) {
        console.error('Erro ao carregar tabelas e colunas:', error)
        alert('Erro ao carregar dados. Verifique sua conexão e autenticação.')
      } finally {
        setLoading(false)
      }
    }
  ...
  // Mapeia as operações do frontend para o formato da API - movido para useMemo
