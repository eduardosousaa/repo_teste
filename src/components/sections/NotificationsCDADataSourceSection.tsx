...
import { FrequenciaItem, CDADataSourceData } from "@/interfaces/Notifications"

interface NotificationsCDADataSourceSectionProps {
  onDataChange?: (data: CDADataSourceData) => void
  initialData?: Partial<CDADataSourceData>
}

export default function NotificationsCDADataSourceSection({ onDataChange, initialData }: NotificationsCDADataSourceSectionProps) {
  const [data, setData] = useState<CDADataSourceData>({
    dadosTipo: "CDA",
    fonteDadosTipo: "API",
    usuario: "",
    senha: "",
  ...
  const handleDataChange = <K extends keyof CDADataSourceData>(field: K, value: CDADataSourceData[K]) => {
  ...
  const handleFrequenciaChange = (field: keyof FrequenciaItem, value: string | number) => {
    ...
    if (data.frequenciaTipo === "DIARIO") {
      ...
      // MENSAL: diaSemana=0, diaAno preenchido (string com dias separados)
      newFrequencia[0] = {
        diaSemana: 0,
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
      <h3 className="text-lg font-medium text-gray-600 mb-4">Fontes de Dados de CDAs</h3>
        ...
        {["API", "RPA"].map((type) => (
          <label key={type} className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              name="cda-source"
              value={type}
              checked={data.fonteDadosTipo === type}
              onChange={() => handleDataChange("fonteDadosTipo", type as "API" | "RPA")}
              className="w-4 h-4 text-blue-600"
            />
            <span className="text-sm text-gray-700">{type === "API" ? "API SEFAZ" : "RPA (Planilhas)"}</span>
          </label>
        ))}
      </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => handleFrequenciaTipoChange(e.target.value as CDADataSourceData["frequenciaTipo"])}
